    ...
    try {
      ...
      if (job.type === 'followup' && job.campaignRef?.campaignId) {
        ...
        try {
          ...
          const retryAt = new Date(nowMs + 60 * 60 * 1000);
          await Outbox.findByIdAndUpdate(job._id, {
            $set: {
              status: 'pending',
              notBefore: retryAt,
              lastError: `Reply check failed: ${errorMsg}`.slice(0, 500),
            },
            $unset: { claimedAt: '', workerId: '' },
          });
          console.warn(`⚠️  Reply check failed for ${job.to} - rescheduled follow-up to ${retryAt.toISOString()}`);
          continue;
        }
      }
      ...
      const resumeTrackingEnabled = process.env.RESUME_TRACKING_ENABLED !== 'false';
